...
namespace BWUK
{
  ...
  /* \brief Represents a set of units. Contains many utility functions to perform an action on the entire unit group.
   *        Backed by a std::set<BWAPI::Unit>
   */
  class UnitGroup : public std::set< BWAPI::Unit >
  {
    public:
      ...
      /** \brief Orders the unit to attack move to the specified location. */
      bool attack( BWAPI::Position target, bool shiftQueueCommand = false ) const;

      /** \brief Orders the unit to attack the specified unit. */
      bool attack( BWAPI::Unit target, bool shiftQueueCommand = false ) const;

      /** \brief Orders the unit to build the given unit type at the given position. Note that if the player does not
      ...
      /** \brief Orders the unit to build the given addon. The unit must be a Terran building that can have an addon
       * and the specified unit type must be an addon unit type. */
      bool buildAddon( BWAPI::UnitType type ) const;

      /** \brief Orders this unit to add the specified unit type to the training queue. Note that the player must
      ...
      /** \brief Orders the unit to morph into the specified unit type. Returns false if given a wrong type.
       * \see Unit::cancelMorph, Unit::isMorphing. */
      bool morph( BWAPI::UnitType type ) const;

      /** \brief Orders the unit to research the given tech type.
      ...
      /** \brief Orders the unit to upgrade the given upgrade type.
      ...
      /** \brief Orders the unit to repair the specified unit. Only Terran SCVs can be ordered to repair, and the
      ...
      /** \brief Orders the unit to burrow. Either the unit must be a Zerg Lurker, or the unit must be a Zerg ground
       * unit and burrow tech must be researched.
       * \see: Unit::unburrow, Unit::isBurrowed. */
      bool burrow() const;

      /** \brief Orders the burrowed unit to unburrow.
       * \see: Unit::burrow, Unit::isBurrowed.
       * */
      bool unburrow() const;

      /** \brief Orders the unit to cloak.
       * \see: Unit::decloak, Unit::isCloaked. */
      bool cloak() const;

      /** \brief Orders the unit to decloak.
       * \see: Unit::cloak, Unit::isCloaked. */
      bool decloak() const;

      /** \brief Orders the unit to siege. Note: unit must be a Terran siege tank.
       * \see Unit::unsiege, Unit::isSieged. */
      bool siege() const;

      /** \brief Orders the unit to unsiege. Note: unit must be a Terran siege tank.
       * \see: Unit::unsiege, Unit::isSieged. */
      bool unsiege() const;

      /** \brief Orders the unit to lift. Note: unit must be a Terran building that can be lifted.
       * \see Unit::land, Unit::isLifted.  */
      bool lift() const;

      /** \brief Orders the unit to land. Note: unit must be a Terran building that is currently lifted.
       * \see Unit::lift, Unit::isLifted. */
      bool land( BWAPI::TilePosition target ) const;

      /** \brief Orders the unit to load the target unit.
      ...
      /** \brief Orders the unit to unload the target unit.
       * \see Unit::load, Unit::unloadAll, Unit::getLoadedUnits, Unit:isLoaded. */
      bool unload( BWAPI::Unit target ) const;

      /** \brief Orders the unit to unload all loaded units at the unit's current position.
      ...
      /** \brief Orders the unit to unload all loaded units at the specified location. Unit should be a Terran
      ...
      /** \brief Works like the right click in the GUI. */
      bool rightClick( BWAPI::Position target, bool shiftQueueCommand = false ) const;

      /** \brief Works like the right click in the GUI. Right click on a mineral patch to order a worker to mine,
       * right click on an enemy to attack it. */
      bool rightClick( BWAPI::Unit target, bool shiftQueueCommand = false ) const;

      /** \brief Orders the SCV to stop constructing the building, and the building is left in a partially complete
       * state until it is canceled, destroyed, or completed.
       * \see Unit::isConstructing. */
      bool haltConstruction() const;

      /** \brief Orders the building to stop being constructed.
       * \see Unit::beingConstructed. */
      bool cancelConstruction() const;

      /** \brief Orders the unit to stop making the addon. */
      bool cancelAddon() const;

      /** \brief Orders the unit to remove the specified unit from its training queue.
       * \see Unit::train, Unit::cancelTrain, Unit::isTraining, Unit::getTrainingQueue. */
      bool cancelTrain( int slot = -2 ) const;

      /** \brief Orders the unit to stop morphing.
       * \see Unit::morph, Unit::isMorphing. */
      bool cancelMorph() const;

      /** \brief Orders the unit to cancel a research in progress.
       * \see Unit::research, Unit::isResearching, Unit::getTech. */
      bool cancelResearch() const;

      /** \brief Orders the unit to cancel an upgrade in progress.
       * \see Unit::upgrade, Unit::isUpgrading, Unit::getUpgrade. */
      bool cancelUpgrade() const;

      /** \brief Orders the unit to use a tech not requiring a target ( ie Stim Pack ). Returns true if it is a valid
       * tech. */
      bool useTech( BWAPI::TechType tech ) const;

      /** \brief Orders the unit to use a tech requiring a position target ( ie Dark Swarm ). Returns true if it is a
       * valid tech.*/
      bool useTech( BWAPI::TechType tech, BWAPI::Position target ) const;

      /** \brief Orders the unit to use a tech requiring a unit target ( ie Irradiate ). Returns true if it is a valid
       * tech.*/
